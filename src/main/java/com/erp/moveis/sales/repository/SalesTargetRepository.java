...
import com.erp.moveis.sales.model.SalesTarget;
...
public interface SalesTargetRepository extends JpaRepository<SalesTarget, Long> {
    ...
    @Query("SELECT st FROM SalesTarget st WHERE st.seller.id = :sellerId AND :date BETWEEN st.periodStart AND st.periodEnd AND st.status = 'ACTIVE'")
    List<SalesTarget> findActiveTargetsForSellerAtDate(
            @Param("sellerId") Long sellerId,
            @Param("date") LocalDate date
    );

    @Query("SELECT st FROM SalesTarget st WHERE st.company.id = :companyId AND :date BETWEEN st.periodStart AND st.periodEnd AND st.status = 'ACTIVE'")
