...
class CommissionServiceImplTest {
    ...
    void shouldGetBySeller() {
        when(commissionRepository.findBySellerId(10L)).thenReturn(List.of(commission));
        try (MockedStatic<CommissionMapper> mapper = mockStatic(CommissionMapper.class)) {
            mapper.when(() -> CommissionMapper.toResponse(commission)).thenReturn(response);
    ...
    void shouldGetByStatus() {
        when(commissionRepository.findByStatus(CommissionStatus.PENDING)).thenReturn(List.of(commission));
        try (MockedStatic<CommissionMapper> mapper = mockStatic(CommissionMapper.class)) {
            mapper.when(() -> CommissionMapper.toResponse(commission)).thenReturn(response);
    ...
    void shouldGetPendingBySeller() {
        when(commissionRepository.findBySellerIdAndStatus(10L, CommissionStatus.PENDING))
    ...
    void shouldGetTotalPaid() {
        when(commissionRepository.calculateTotalPaidBySeller(10L)).thenReturn(new BigDecimal("15000"));
    ...
    void shouldGetDueForPayment() {
        when(commissionRepository.findDueForPayment(any(LocalDate.class))).thenReturn(List.of(commission));
        try (MockedStatic<CommissionMapper> mapper = mockStatic(CommissionMapper.class)) {
            mapper.when(() -> CommissionMapper.toResponse(commission)).thenReturn(response);
